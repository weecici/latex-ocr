...
from models.ResNet18_LSTM import ResNet18_LSTM
...
def load_model_and_producer(model_name):
    ...
    if model_name == "CNN & LSTM":
        model = CNN_LSTM(vocab_size, ModelConfig.CONFIG["word_emb_dim"], ModelConfig.CONFIG["rnn_h_dim"], ModelConfig.CONFIG["rnn_o_dim"], ModelConfig.CONFIG["enc_out_dim"], ModelConfig.CONFIG["attn_dim"], ModelConfig.CONFIG["dropout"])
    else:
        model = ResNet18_LSTM(vocab_size, ModelConfig.CONFIG["word_emb_dim"], ModelConfig.CONFIG["rnn_h_dim"], ModelConfig.CONFIG["rnn_o_dim"], ModelConfig.CONFIG["enc_out_dim"], ModelConfig.CONFIG["attn_dim"], ModelConfig.CONFIG["dropout"])
    model.load_state_dict(torch.load(weight_path, map_location='cpu'))
    # 3. Khởi tạo Producer
    producer = LatexProducer(model, vocab, max_len=current_config["max_len"], use_cuda=torch.cuda.is_available(), beam_size=current_config["beam_size"])
    return producer
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Chuyển đổi (Predict)"):
        with st.spinner("Đang xử lý..."):
            ...
            # Thông thường với CRNN, H_prime = H // 16, W_prime = W // 8 hoặc tương tự
