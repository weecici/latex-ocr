...
from .model import START_TOKEN_ID, END_TOKEN_ID, PAD_TOKEN_ID
...
def smart_resize(img, max_side=800):
    """
    Resize image if the largest side exceeds max_side.
    Keep Aspect Ratio.
    """
    w, h = img.size
...
def inference(model, vocab, image, device, beam_size=5, max_len=150):
    producer = LatexProducer(
        model,
        vocab,
        beam_size=beam_size,
        max_len=max_len,
    ...
    # Accept either a file path or a PIL.Image.Image
